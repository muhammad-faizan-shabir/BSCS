...
int search(char paragraph[],char substring[])
{
	...
	else
	{
		...
		{
			if ((paragraph[i] == ' ') || (counter == lengthOfSubstring )) // checking for spcace character and if counter exceeds the length of substring
			{
				counter = 0;
			}
			...
			if (counter == lengthOfSubstring ) // checking if there is a match of the substring
			{
				occurence++;
			}
		}
