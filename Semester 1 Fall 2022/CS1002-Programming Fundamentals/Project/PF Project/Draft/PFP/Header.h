...
void pos_val(int direction,int length,int temp_x[],int temp_y[],int s_pos_X[],int s_pos_Y[],int &head_x,int &head_y)
{
	if (direction == 6)
	{
...
void check(int length, int head_x, int head_y,int s_pos_X[],int s_pos_Y[],int &play)
{
	for (int i = 1; i < length; i++)
	{
...
void game_play(int &length, int &f_PosX, int &f_PosY, int &direction, int &head_x, int &head_y, int &play, int s_pos_X[], int s_pos_Y[], int temp_x[], int temp_y[], int &score, int &delay, int &counter, char &ruc, char &hl, char &vl, char &luc, char &ldc, char &rdc,
	char &tu, char &td, char &food, char &sn, char &key, char snake[])
{
	...
	while (play == 1)
	{
		...
		check(length,head_x,head_y,s_pos_X,s_pos_Y,play);
		sn_eat(head_x,head_y,f_PosX,f_PosY,length,s_pos_X,s_pos_Y,snake,sn,score,counter);
		pos_val(direction,length,temp_x,temp_y,s_pos_X,s_pos_Y,head_x,head_y);
		//Sleep(500);
		/*if (play == 1)
		{
			system("cls");
